...
export default function UsersView({
  ...
  const openEditScopeModal = (user: User) => {
    setEditingUser(user);
    setRole(user.role || "surveyor");
    const existingScopes: ScopeSelection[] = (user.scopes || []).map((s) => ({
      propertyId: s.propertyId,
      buildingId: s.buildingId,
      floorId: s.floorId,
      roomId: s.roomId,
      label: [s.propertyName, s.buildingName, s.floorName, s.roomName]
        .filter(Boolean)
        .join(" > "),
    }));
          ...
          (s) =>
            !(
              (s.propertyId || undefined) ===
                (scopeObj.propertyId || undefined) &&
              (s.buildingId || undefined) ===
                (scopeObj.buildingId || undefined) &&
              (s.floorId || undefined) === (scopeObj.floorId || undefined) &&
              (s.roomId || undefined) === (scopeObj.roomId || undefined)
            ),
        );
    ...
    <div className="p-8 max-w-7xl mx-auto text-slate-100">
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <button
          ...
          className="bg-sky-600 hover:bg-sky-500 text-white font-semibold px-4 py-2.5 rounded-lg text-sm transition shadow-lg shadow-sky-600/25 cursor-pointer"
        >
          + Add New User
        </button>
      </div>
          ...
          <thead className="bg-slate-950 text-slate-400 text-xs uppercase border-b border-slate-800">
            <tr>
              ...
              <th className="py-3.5 px-4">
                Scopes (Property / Building / Floor / Room)
              </th>
            ...
            {users.map((user) => {
              ...
              return (
                <tr key={user.id} className="hover:bg-slate-800/40">
                  ...
                  <td className="py-3.5 px-4 uppercase text-xs font-semibold text-sky-400">
                    {user.role}
                  </td>
                  <td className="py-3.5 px-4">
                    <span
                      className={`text-xs px-2.5 py-0.5 rounded-full ${isActive ? "bg-emerald-500/10 text-emerald-400" : "bg-rose-500/10 text-rose-400"}`}
                    >
      ...
      {(isCreateModalOpen || editingUser) && !otpModalInfo && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 w-full max-w-2xl shadow-2xl">
            <form
              onSubmit={editingUser ? handleUpdateUserScopes : handleCreateUser}
              className="space-y-4"
            >
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-sky-500 cursor-pointer"
                >
                  {SYSTEM_ROLES.map((r) => (
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-2">
                  Assign Granular Scope (Property / Building / Floor / Room)
                </label>
                <div className="max-h-72 overflow-y-auto bg-slate-950 border border-slate-800 rounded-lg p-3 space-y-3">
                  {initialProperties.map((prop) => {
                    const propBuildings = initialBuildings.filter(
                      (b) => b.propertyId === prop.id,
                    );
                    return (
                      <div
                        key={prop.id}
                      ...
                      >
                        ...
                        <div className="pl-4 mt-2 space-y-2">
                          {propBuildings.map((bldg) => {
                            const bldgFloors = initialFloors.filter(
                              (f) => f.buildingId === bldg.id,
                            );
                            return (
                              <div key={bldg.id}>
                                ...
                                <div className="pl-4 mt-1 space-y-1">
                                  {bldgFloors.map((flr) => {
                                    const flrRooms = initialRooms.filter(
                                      (r) => r.floorId === flr.id,
                                    );
                                    return (
                                      <div key={flr.id} className="text-xs">
                                          ...
                                          <input
                                            ...
                                            onChange={() =>
                                              toggleScope({
                                                ...
                                                buildingId: bldg.id,
                                                floorId: flr.id,
                                                label: `${prop.name} > ${bldg.name} > ${flr.name}`,
                                              })
                                            }
                                          />
                                        ...
                                        <div className="pl-4 flex flex-wrap gap-2 mt-1">
                                          {flrRooms.map((room) => (
                                            <label
                                              key={room.id}
                                              className="flex items-center gap-1 text-[11px] bg-slate-900 px-2 py-0.5 rounded border border-slate-800 text-slate-400 cursor-pointer"
                                            >
                                              <input
                                                type="checkbox"
                                                checked={isScopeSelected(
                                                ...
                                                onChange={() =>
                                                  toggleScope({
                                                    ...
                                                    floorId: flr.id,
                                                    roomId: room.id,
                                                    label: `${prop.name} > ${bldg.name} > ${flr.name} > ${room.name}`,
                                                  })
                                                }
                                              />
                                              <span>{room.name}</span>
              ...
              <div className="flex gap-3 pt-4">
                <button
                  type="button"
                  onClick={closeModal}
                  className="flex-1 bg-slate-800 hover:bg-slate-700 py-2.5 rounded-lg text-sm cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex-1 bg-sky-600 hover:bg-sky-500 py-2.5 rounded-lg text-sm font-semibold cursor-pointer"
                >
                  {submitting ? "Saving..." : "Save User & Scopes"}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 w-full max-w-md shadow-2xl text-center">
            ...
            <p className="text-sm text-slate-400 mb-4">
              Share this one-time access code with{" "}
            ...
            <button
              onClick={closeModal}
              className="w-full bg-sky-600 hover:bg-sky-500 py-2.5 rounded-lg text-sm font-semibold text-white cursor-pointer"
            >
              Done
            </button>
          </div>
